...
--      ⚠️마스터 자체 결함 = 이중(검수회신 track7_b1d1 §3 병기 지시, 260731 실측 확정):
--      ⓐ조회키 $I$2가 제목 병합(B1:I2) 내부라 자동조회 미작동 ⓑVLOOKUP 참조 테이블
--      자체가 #REF! 소실(전 8칸 공통 = IF($I$2="","",VLOOKUP($I$2,#REF!,n,0))) —
--      정본 교정 시 키 셀 위치·참조 테이블 복원 둘 다 필요(반쪽 수리 방지). 시트 교정은
--      정본 관리 영역(규정 원문·H3100 코드중복 정본 정비 동선 편승 후보). 앱 매핑 =
--      로그 2종 grid만(사용/불출 25~27 · 교정/수리 이력 30~36) + 관리번호·계측기명은
--      앱 기록용(의도적 unmapped, 식별).
...
-- ── ② L3100-02 계측기 이력카드 ──
UPDATE forms SET
  description='재설계 — 7차 트랙(0119, 260731): 계측기 이력카드. 사용/불출 현황 grid(25~27행: 일자·부서명·용도·비고) + 교정(점검)/수리 이력 grid(30~36행: 일자·이력사항·검교정결과·기관·차기교정일·담당확인·비고). 관리번호·계측기명은 앱 기록용(헤더 8칸은 VLOOKUP 자동조회 수식 — 매핑 금지). ⚠️마스터 결함 기록(이중, 검수회신 260731 확정): ⓐ조회키 $I$2가 제목 병합(B1:I2) 내부라 자동조회 미작동 ⓑVLOOKUP 참조 테이블 #REF! 소실(전 8칸 공통) — 정본 교정 시 키 위치·참조 테이블 복원 둘 다 필요. 시트 교정은 정본 관리 영역. 사진·헤더는 시트 몫.'
WHERE code='L3100-02';
...
INSERT INTO form_change_log (form_code, changed_on, change_type, old_value, new_value, reason, migration) VALUES
 ...
 ('L3100-02','2026-07-31','cellmap_redesign','0019 잔재 7필드(충돌 4 = VLOOKUP 조회 셀 F4~F7 매핑)','로그 grid 2종(사용/불출 3행·교정/수리 7행) + 관리번호·계측기명 앱 기록(의도적 unmapped)',
  '헤더 8칸 = VLOOKUP($I$2) 자동조회 수식 — 입력 셀 아님. 마스터 결함 이중 기록(ⓐ조회키가 제목 병합 내부 ⓑ참조 테이블 #REF! 소실 — 검수회신 track7_b1d1 §3 병기)·시트 교정은 정본 관리 영역','0119'),
