...
**내일 선택지(사용자 미결정 — 추천 A)**:
- **(A) 자주검사 체크시트 파일럿** ⭐추천: 사용자가 MES 화면자료 복붙 제공 → CSV 저장 → 항목 자동분배 → 격자 작성양식 → (필요시 이미지) → 공식 엑셀 출력. "작성 불편" 직접 해소 + 실데이터 증명. (격자 엔진 0030/0031 + ExcelPasteModal 재활용)
- **(B) CP-03 성과지표 8종 반영**: process_revisions/process_doc 채움(작은 작업, 바로 보임).
- **(C) 나머지 프로세스 8장 + 규정 PDF 쭉 읽어** 전체 연결고리·갭 지도 완성.
