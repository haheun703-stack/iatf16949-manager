...
**▶ 다음**:
1. **사장님 스팟체크**: 대시보드 v3 실사용 확인(슬라이서·KPI 클릭 내비·TOP [AI 초안›]·히트맵 핫스팟 클릭→AI 접이식·궤적). 피드백 → 다듬기.
2. 검수 잔여: 🟡죽은코드 2덩어리(domain/ 11파일·register.ts v4 19채널 540줄=삭제안전 검증됨) + TPC 하드코딩 나머지(회사명·프롬프트 컨텍스트 → 제품화 팩 분리와 묶어).
3. 개발팀 감사 잔여: 부실 셀맵 5건 보강. / 팀원 피드백 → 지시서 Phase 3(사이드바 5방+렌즈 토글) 판단.
4. 히트맵 핫스팟→모의심사 직접 연결 고도화(현재는 AI 접이식 열기까지 — MockAuditCard에 initialItemKey prop 후보).
