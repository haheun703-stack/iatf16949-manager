...
function Panel({
  ...
  return (
    <div className={cn('bg-card border border-border rounded-xl shadow-sm p-4 flex flex-col', className)}>
      <h2 className="text-[13px] font-extrabold">{title}</h2>
      {sub && <div className="text-[10px] text-muted-foreground mb-2">{sub}</div>}
      {children}
    </div>
  )
}
...
export function TeamRail(): JSX.Element {
  ...
  return (
    <div className="bg-card border border-border rounded-xl shadow-sm p-2.5 flex flex-col gap-1.5 lg:row-span-2">
      <div className="text-[11px] font-extrabold text-muted-foreground px-1 mb-0.5">팀</div>
      <button
        type="button"
        onClick={() => setPage('home')}
        className="text-[12px] font-bold text-white rounded-lg py-2 px-1.5 hover:opacity-90 transition-opacity"
      ...
      {TEAMS.map((t) => (
        <button
          ...
          className="text-[12px] font-bold rounded-lg py-2 px-1.5 hover:opacity-80 transition-opacity"
  ...
  return (
    <Panel title="미충족 분포 — 카테고리" sub={`${itemTotal}항목 중 미충족 ${total} · 항목 수 기준`}>
      <div className="flex items-center gap-4 flex-1">
        <div
          className="w-[140px] h-[140px] rounded-full relative shrink-0"
        ...
        >
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <b className="text-[24px] font-extrabold" style={{ color: STATUS.critical }}>
              {total}
            </b>
            <span className="text-[9.5px] text-muted-foreground">미충족</span>
          </div>
        </div>
...
export function TeamSignalColumns({
  ...
  const seg = (n: number, color: string, ink = '#fff'): JSX.Element | null =>
    n > 0 ? (
      <div
        className="rounded-[3px] flex items-center justify-center text-[9.5px] font-extrabold"
        style={{ height: `${(n / max) * 100}%`, minHeight: 8, backgroundColor: color, color: ink }}
      >
        {n}
      </div>
    ) : null
  ...
  return (
    <Panel title="팀별 SQ 신호등" sub="항목 수 · 미충족 많은 순">
      <div className="flex items-end gap-2.5 flex-1 min-h-[130px] relative px-1">
        {[25, 50, 75].map((p) => (
          <div
            key={p}
      ...
      <div className="flex gap-2.5 px-1 pt-1.5">
        {cols.map((c) => (
          <span key={c.id} className="flex-1 text-center text-[10px] font-bold text-muted-foreground truncate">
            {c.label}
          </span>
        ))}
      </div>
      <div className="flex gap-3 flex-wrap text-[10px] text-muted-foreground pt-2">
        {(
          [
            ['충족', STATUS.good],
            ['진행', STATUS.warning],
        ...
        <table className="w-full border-collapse tabular-nums">
          <thead>
            <tr>
              <th />
              {catNames.map((n) => (
                <th key={n} className="text-[10px] font-extrabold text-muted-foreground px-1 py-1 text-center">
                  {n.replace('관리', '').replace('체제', '')}
                </th>
              ))}
              <th className="text-[10px] font-extrabold text-muted-foreground px-1 py-1">계</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.id}>
                <th className="text-right text-[11px] font-bold text-muted-foreground whitespace-nowrap pr-1.5 py-0.5">
                  {r.label}
                </th>
                {r.cells.map((v, ci) => {
                  ...
                  return (
                    <td key={ci} className="p-[2px]">
                      <div
                        ...
                        className={cn(
                          'rounded-[4px] text-center text-[11px] font-bold py-1',
                ...
                <td className="text-center text-[11px] font-extrabold py-0.5">{r.total}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr>
              <th className="text-right text-[11px] font-bold text-muted-foreground pr-1.5 py-1">계</th>
              {colTotals.map((v, ci) => (
                <td key={ci} className="text-center text-[11px] font-extrabold py-1">
                  {v}
                </td>
              ))}
              <td className="text-center text-[11px] font-extrabold py-1">
  ...
  return (
    <Panel title="미충족 TOP — 배점순" sub="여기부터 잡으면 점수가 가장 빨리 오릅니다">
      <div className="flex flex-col gap-2 flex-1 justify-center">
        {top.length === 0 && (
          <div className="text-[11px] text-muted-foreground text-center py-4">미충족 항목 없음 🎉</div>
        )}
        {top.map((i) => (
          <div key={i.code} className="grid grid-cols-[34px_1fr_40px_60px] gap-2 items-center">
            <button
              type="button"
              onClick={() => setPage('sq-readiness')}
              className="text-[10.5px] font-extrabold text-left hover:underline"
              style={{ color: STATUS.critical }}
            >
              {i.code}
            </button>
            <div className="h-[14px] rounded bg-muted overflow-hidden">
              <div
                className="h-full rounded"
                style={{ width: `${(i.points / max) * 100}%`, backgroundColor: STATUS.critical }}
              />
            </div>
            <div className="text-[11px] font-extrabold text-right tabular-nums">{i.points}점</div>
            <button
              type="button"
              onClick={() => openAuthor(true)}
              className="text-[9.5px] font-extrabold px-2 py-[3px] rounded-full bg-sky-100 text-sky-700 border border-transparent hover:border-sky-500 whitespace-nowrap"
            >
              AI 초안 ›
            </button>
            <div className="col-span-4 -mt-1 text-[10px] text-muted-foreground truncate">{i.title}</div>
          </div>
        ))}
      </div>
    </Panel>
  ...
  return (
    ...
    >
      <div className="flex-1 flex flex-col justify-end">
        <svg viewBox="0 0 300 120" preserveAspectRatio="none" className="w-full h-[120px] block" role="img" aria-label="준비도 궤적: 현 페이스 대 필요 페이스">
        ...
        <div className="flex px-2 pt-1">
          {monthLabels.map((m) => (
            <span key={m} className="flex-1 text-center text-[10px] font-semibold text-muted-foreground">
              {m}
            </span>
          ))}
        </div>
        <div className="flex gap-3 flex-wrap text-[10px] text-muted-foreground pt-1.5">
          <span>
    ...
    <Panel title="팀별 책임 양식" sub="막대 = 보유 양식 · 진한 부분 = 작성가능(셀맵 정의)">
      <div className="flex flex-col gap-1.5 flex-1 justify-center">
        {rows.map((r) => {
          ...
          return (
            <div key={r.teamId} className="grid grid-cols-[88px_1fr_86px] gap-2 items-center">
              <div className="text-[11px] font-bold text-muted-foreground text-right truncate">{label}</div>
              <div className="flex justify-center">
                <div
                  className="h-[16px] rounded relative overflow-hidden"
                  style={{ width: `${(r.formsTotal / max) * 100}%`, backgroundColor: '#9ec5f4', minWidth: 24 }}
                >
                  <div
