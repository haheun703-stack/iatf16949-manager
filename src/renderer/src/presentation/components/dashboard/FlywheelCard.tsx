  ...
  return (
    <div className="bg-card border border-border rounded-xl shadow-sm p-5">
      <h2 className="text-sm font-bold flex items-center gap-1.5 mb-3">
        ...
        AI 수용률 · 비용 <span className="text-[11px] font-normal text-muted-foreground">쓸수록 정확해지는 지표</span>
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <div className="flex items-baseline gap-2 mb-1.5">
            ...
            <span className="text-[12px] text-muted-foreground">수용률{decided ? ` (결재 ${decided}건)` : ''}</span>
          </div>
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-[12px]">
            <span className="text-muted-foreground">제안 <b className="text-foreground">{s.proposed + s.approved + s.rejected}</b></span>
            <span className="text-emerald-600">승인 {s.approved} <span className="text-[10px]">(그대로 {s.approvedAsIs}·수정 {s.approvedEdited})</span></span>
        ...
        <div className="md:border-l md:border-border md:pl-4">
          <div className="flex items-baseline gap-2 mb-1.5">
            ...
            <span className="text-[12px] text-muted-foreground">누적 · {s.cost.totalCalls}콜</span>
          </div>
          <div className="space-y-0.5">
            {s.cost.byPurpose.length === 0 ? (
              <p className="text-[11px] text-muted-foreground">아직 AI 호출 기록 없음</p>
            ) : (
